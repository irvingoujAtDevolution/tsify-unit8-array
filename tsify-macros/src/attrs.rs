...
impl TsifyContainerAttrs {
    ...
    pub fn from_nested_meta(
        ...
        if meta.path.is_ident("missing_as_null") {
            if self.ty_config.missing_as_null {
                return Err(meta.error("duplicate attribute"));
            }
            if cfg!(not(feature = "js")) {
                return Err(meta.error("#[tsify(missing_as_null)] requires the `js` feature"));
            }
            self.ty_config.missing_as_null = true;
            return Ok(());
        }

        if meta.path.is_ident("hashmap_as_object") {
            if self.ty_config.hashmap_as_object {
                return Err(meta.error("duplicate attribute"));
            }
            if cfg!(not(feature = "js")) {
                return Err(meta.error("#[tsify(hashmap_as_object)] requires the `js` feature"));
            }
            self.ty_config.hashmap_as_object = true;
            return Ok(());
        }
